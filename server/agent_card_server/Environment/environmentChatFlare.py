...
class ProjectedWorkSpace(BaseModel):
    ...
    projected_position: Dict[str, List[float]] # {docstore_id: (x, y)}
    class Config:
        arbitrary_types_allowed = True
...
class InformationSeekingEnvironment(BaseEnvironment): 
    ...
    def create_sub_env(self, sourceEnv: BaseEnvironment, documentIds: List[str], subEnvName:str, cardId: str): 
        ...
        self.projected_workspace = sourceEnv.projected_workspace
        remained_projected_positions = {}
        for id, pos in self.projected_workspace.projected_position.item():
            if id in documentIds:
                remained_projected_positions[id] = pos
        self.projected_workspace.projected_position = remained_projected_positions
