...
def generate_circular_grid(step, layer, radius=1, skip=1):
    ...
    return array(points), weights_radius, points_map
...
class CircularSom(object):
    ...
    def __init__(self, step, layer, input_len, sigma=1.0, learning_rate=0.5,
        ...
        self._grid, self._weights_radius, self._grid_map = generate_circular_grid(step, layer)
    ...
    def train(self, data, relevance_score, num_iteration, w_s=.4, w_r=.6, random_order=False, client_handler=None, cardId=None, verbose=False, use_epochs=False):
        ...
        for t, iteration in enumerate(iterations):
            ...
            if iteration == 0: 
                self.start_a_new_epoch() 
            self.update(data[iteration], self.winner(data[iteration], relevance_score[iteration]),
